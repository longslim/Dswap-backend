...
// ZeroHash request signer (HMAC-SHA256). Many ZeroHash docs use HMAC SHA256 over timestamp + method + path + body
...
// Create a deposit address mapped to your platform. ZeroHash supports providing an "account_label" or "reference" to identify user.
...
// Get vault balances (platform)
async function getPlatformBalances() {
  const path = '/fund/accounts'; // endpoint may vary; adapt if your ZeroHash plan uses other endpoint
